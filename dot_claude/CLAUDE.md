...
`~/.claude/rules/` 配下（`alwaysApply: true`）は全プロジェクトで読み込まれる。以下を厳守する:

- **原則のみ書く**: 特定サイト名 / URL / 企業名 / 個別プロジェクト固有の事例は入れない
- **具体例が必要ならパターン名で抽象化**: ❌「特定サービスで見た事例」→ ✅「〇〇型のパターン」
- **編集前に自問**: 「この文言は全ての利用プロジェクトで正当か？」
- **個別事例の置き場所**: プロジェクト `CLAUDE.md` / skill 内 / プロジェクト内メモ。ルール本体には置かない
