...
テスト駆動開発（TDD）のワークフローをガイドするスキル。

**superpowers:test-driven-development** の原則に従いつつ、以下を追加で提供する：
...
### 基本原則（superpowers:test-driven-development と共通）

> 失敗するテストを先に書いてからプロダクションコードを書く。テストが先にあることで、実装が要件から逸脱するリスクを防げる。
...
実装を先に書いてしまった場合:
- 実装を削除し、テストから書き直す。前の実装を参考にすると、テストが実装に引きずられてコンテキスト汚染が起きるため、新鮮な視点で書く
