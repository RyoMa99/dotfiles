...
user_invocable: true
allowed-tools: ["Bash(git:*)", "Bash(gh:*)", "Bash(copilot:*)", "Bash(gemini:*)", "Bash(cd:*)", "Bash(wc:*)", "Bash(head:*)", "Bash(cat:*)", "Bash(echo:*)", "Bash(test:*)", "Bash(find:*)", "Bash(ls:*)", "Bash(pnpm:*)", "Bash(npx:*)", "Read", "Edit", "Write", "Glob", "Grep", "Agent", "Skill(coderabbit:review)"]
---
...
| PR URL (`https://github.com/.../pull/123`) | URL から PR 番号を抽出し `gh pr diff 123` |
| ブランチ名 (`fix/some-feature` 等) | `gh pr list --head <branch> --json number` で PR 番号を解決し `gh pr diff` |

### PR レビュー時の追加情報取得

レビュー対象が PR（PR番号、PR URL、ブランチ名→PR解決）の場合、差分に加えて以下の情報も取得してレビュアーに渡す:

```bash
# PR の基本情報（タイトル、本文、ラベル）
gh pr view <number> --json title,body,labels

# PR の既存レビューコメント（他のレビュアーの指摘を把握）
...
これらの情報をレビュアーに渡すことで、既存の議論やコンテキストを踏まえたレビューが可能になる。
...
妥当と判断した指摘の一覧を提示し、`AskUserQuestion` で次のアクションを選択してもらう:

1. **修正する** — 妥当な指摘を自動修正し、テストで検証する
2. **列挙のみ** — 指摘の一覧をサマリーとして出力し、修正は行わない

### 3-4. 修正の実行（「修正する」を選択した場合のみ）

妥当と判断した指摘のみ修正を実施する。修正後、変更が既存テストを壊していないか確認する（テストランナーが利用可能な場合）。

### 3-5. PR レビュー時の投稿（PR 対象の場合のみ）

レビュー対象が PR の場合、サマリー出力後に GitHub への投稿を提案する。

1. サマリーをユーザーに提示する
2. 「この内容で PR にレビューコメントを投稿しますか？」と `AskUserQuestion` で確認する
3. ユーザーが承認した場合のみ、`gh api` でインラインコメント付きレビューを投稿する
4. 投稿後、`gh pr edit --add-label "ai-review"` でラベルを付与する

**投稿前の確認なしに PR へコメントを投稿してはならない。**

### 3-6. サマリー出力

修正完了後（または PR レビューの場合は投稿判断後）、以下の形式でサマリーを出力する。サマリーの目的は「修正判断が適切だったかをユーザーがダブルチェックできること」。
