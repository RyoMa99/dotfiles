...
計画ファイルを `docs/plan/{ブランチslug}.md` に保存し、実装前のスナップショットとしてコミットする。
ブランチ名と対応させることで追跡しやすくなる。

```bash
# 例: ブランチが feature/user-auth なら docs/plan/user-auth.md
git add docs/plan/
...
#### 4-5. 計画ファイルの削除（PR 作成後）

PR を作成して計画の役目を終えたら、計画ファイルを削除する（git 履歴には Step 0 のコミットで残っている）。
...
> PR を作成しない場合（「コミットのみ」「コミット + プッシュ」選択時）は計画ファイルを残す。後続セッションで参照する可能性があるため。
