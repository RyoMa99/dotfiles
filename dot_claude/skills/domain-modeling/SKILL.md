...
- **ドメイン記述ミニ言語で半形式記述**（kawasima, Wlaschin 派生）: コードの前段で `data` / `behavior` / `AND` / `OR` / `?` / `->` の記法でユビキタス言語の文法レベルを書き下す。実装と完全一致させる必要はないが、四層すべてを Claude / 設計者 / ドメイン専門家が共有できる中間表現になる。具体記法は `reference.md` の「ドメイン記述ミニ言語」を参照
...
- **翻訳の蓄積**: ドメイン専門家と開発者の語りの差を場面ごとに翻訳し続けている → 言語が分離していて semantics が共有されていない（Evans: "Translation blunts communication and makes knowledge crunching anemic."）
- **瞬間的表現の喪失**: ドメイン理解の核心は発話の場で瞬間的に現れるが、用語集は静的な保存場所であり「文を作って使う場」を持たないため、その瞬間的な言い回しが記録されずに消えていく
- **動詞の欠如**: 用語集が名詞ばかりで振る舞い（動詞）が現れない → behavior が言語の semantics 層を作る。動詞なしでは「語がどう使われてよい/使われてはいけないか」を書けない
- **ありえない状態の許容**: フラグ（boolean）や Optional（`?`）の組み合わせで状態を表現し、業務上ありえない組み合わせを型構造が許してしまう → 状態は `OR` で型に分離する。anemic domain model（Fowler 2003）と同じ問題が言語側で起きている兆候
- **コードとの乖離**: 用語集が静的ドキュメントとして放置され、コードの type / 関数名と並行進化していない（Vernon: "the code is the enduring expression of Ubiquitous Language"）
...
- **tactical pattern バイアス**: Entity / Aggregate / Value Object などの DDD 戦術パターンから設計を始めてしまう。DDD の core はパターン適用ではなくユビキタス言語の設計（Khononov, Comartin）。**まずアクターと業務語彙、振る舞い・状態遷移を明確にしてから、その表現として戦術パターンを選ぶ**順序を保つ
...
- **ミニ言語で書き下す**: 検出した概念を `data X = A AND B`、`behavior 〜する = 入力 -> 出力 OR エラー` の記法で書く（`reference.md` 参照）。書けない・OR の右辺が出てこない・成功失敗が単一型で返るなどが起きたら、その時点で**未発見の概念**が残っているサイン
...
- **状態 → 別の型に分離**: boolean flags / status code → 状態ごとの Discriminated Union（Always-Valid Model）。ミニ言語の指針「**フラグではなく `OR`**」「**ステータスコードではなく状態と遷移を `OR` と `behavior` で明示**」（`reference.md` 参照）に対応
