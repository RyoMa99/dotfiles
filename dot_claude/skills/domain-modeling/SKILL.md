...
- **Anemic Domain Model**: Entity がデータだけで、ロジックが `XxxService` に流出している
...
- **全体統一モデルの神話**: 「全部を一つの統一モデルに集約するのが賢い」と考え、異なるコンテキストの概念を無理に共通化する
...
- **DRY の誤適用**: 同じ名前だからと異なるコンテキストの概念を統合 → 神クラス化。DRY はコードの重複ではなく**ビジネス知識の重複**を防ぐ原則
