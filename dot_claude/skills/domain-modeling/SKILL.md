...
同じ言葉でも**アクターの目的が異なれば意味とルールが変わる**（Wittgenstein「言語ゲーム」）。
この観点で境界を切ると、DDD の境界付けられたコンテキストが自然に導出される。
...
> 同じ「商品」でもアクターの目的が違えば、持つべきプロパティもルールも異なる。

---

## モデルの問い直し（脱構築）

モデリングの初期段階や既存モデルのリファクタリング時に、**既存の概念名に固執せず本質を問い直す**。

> 「みんなは〇〇と呼んでるけど、実は xx という概念が正体なのでは？」

### 問い直しの観点

- **業界用語・社内用語の再検討**: 慣習的な名前が本質を隠していないか
- **複数の責務が1つの名前に混在していないか**: 「注文」が実は「カート」「発注」「配送依頼」の3概念を含んでいないか
- **既存の認知枠組み（スキーマ）への気づき**: 過去の経験や他システムの知識が、現在のドメインの理解を歪めていないか

### いつ問い直すか

- 1つの型が肥大化している（プロパティが増え続ける）
- 型名と実際の振る舞いが乖離している
- 異なるコンテキストで同じ型を無理に共有している
