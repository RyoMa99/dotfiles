...
元ネタ: [mizchi/chezmoi-dotfiles empirical-prompt-tuning](https://github.com/mizchi/chezmoi-dotfiles/blob/main/dot_claude/skills/empirical-prompt-tuning/SKILL.md) / [解説記事](https://zenn.dev/mizchi/articles/empirical-prompt-tuning)
...
- `skill-creator` — 新規スキルの作成・既存スキルの改善。empirical-prompt-tuning と併用することで、作成直後の品質検証ループに組み込める
- `session-retrospective` — タスク後の学び固定化。本 skill はプロンプト開発中、retrospective はタスク終了後、と使い分ける
