...
ユーザーへのフィードバック（成功・エラー・状態通知）は、伝え方で体験が大きく変わる。判断軸は **「ユーザーが次にどう行動できるか」**。

### 5つのパターン

| パターン | 特徴 | 適する場面 | 不適な場面 |
|----------|------|-----------|-----------|
| **トースト通知** | 数秒で自動消滅、操作を妨げない | 保存完了、軽微な成功/エラー | 重大エラー、見落としが致命的な情報 |
| **ダイアログ（モーダル）** | 画面中央・操作をブロック | 不可逆操作の確認、続行不能エラーでの意思決定 | 補助的な通知、頻繁な情報提示 |
| **専用エラー画面** | ページ全体を切り替える | 404、サーバーダウン、権限なし | 部分的なエラー |
| **アラート／通知バナー** | 画面上部に常駐、作業継続可 | ページ全体の状態説明（メンテ予告、読み取り専用、サーバー 5xx 等の再試行可能な一時的失敗） | 即修正可能な入力ミス |
| **インラインエラー** | 該当要素の直下に表示 | フォームの入力ミス、その場で修正可能な問題 | ページ全体に関わる状態 |
...
- **インラインエラー** → 本ファイル「フォームバリデーション」節（`role="alert"`, `aria-describedby`, `aria-invalid`）
- **ダイアログ** → `react-patterns.md` の `useModal` パターン
- **トースト** → Toast プロバイダ経由（テスト例は本ファイル「テスト」節の `ToastProvider` 参照）
- **アラートバナー / 専用エラー画面** → 共通コンポーネント（`components/ui/`）に配置し、feature から利用

### アンチパターン

```tsx
// BAD: 不可逆操作なのにトースト
<Toast message="アカウントを削除しました" />
// → 「次の行動」が確認・取り消しであるはず。削除前にダイアログで確認すべき

// BAD: 即修正可能な入力ミスをトーストで通知
toast.error("メールアドレスの形式が不正です");
// → 該当フィールド直下にインラインエラーを出す

// BAD: サーバーダウンをインラインで表示
<input /><p>サーバーに接続できません</p>
// → ページ全体が機能しないので専用エラー画面かバナー
```
