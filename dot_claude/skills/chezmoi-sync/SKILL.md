...
MISSING=$(brew bundle check --file=~/.Brewfile --verbose 2>&1 | grep "needs to be installed" | sed 's/ needs to be installed or updated//')
...
`brew list` で実際にインストール済みか確認し、ユーザーへの表示を分ける。
