---
name: authorization
description: Use when designing or implementing authorization (access control) in any system. Covers model selection (RBAC/ABAC/ReBAC), XACML architecture (PAP/PIP/PDP/PEP), layer placement in DDD, and structural enforcement patterns. Trigger this skill whenever the user mentions permissions, roles, access control, authorization, guards, policies, or asks "who can do what". Also use when reviewing code that involves @PreAuthorize, Guards, Policies, RLS, or any access control mechanism.
---

# 認可設計

認可（Authorization）の設計判断に必要な知見。モデル選定 -> アーキテクチャ -> 層配置 -> 構造的強制の順で判断する。
...
実装パターン・CQRS 統合・抜け穴チェックリストの詳細は `reference.md` を参照。
