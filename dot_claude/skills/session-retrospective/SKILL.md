...
- **見積もりの妥当性**: PR description の `## Plan` セクション（見積もりメモ含む）を参照し、計画時の判断と実装結果の整合性を検証する。計画ファイルは削除済みのため、PR description が唯一の参照元
- **RDRA 成果物の鮮度**: `docs/rdra/` がある場合のみ。実装を通じて「RDRA の要件（REQ）やビジネスルール（BR）が実態と乖離している」「新しいアクターやUCが必要」と気づいた場合に検出。承認された更新提案は `docs/rdra/overview.md` を直接編集し、`/rdra` の更新フロー（トレーサビリティ再検証）に従って依存チェーンの整合性を確認する
