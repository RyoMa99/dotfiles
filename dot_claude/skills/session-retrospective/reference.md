...
実装を振り返り、変更した領域について「この部分をゼロから再設計するなら？」と問いかける。
現行設計と理想設計の乖離が見つかった場合に記録する。

### Issue 化の判断基準

| 条件 | 対応 |
|------|------|
| 乖離が大きく、今後の開発速度に影響する | GitHub Issue を作成（リファクタラベル付与） |
| 乖離はあるが影響は軽微 | レポートに記録のみ（Issue 化は見送り） |
| 今回の実装で解消済み | 記録不要 |

### Issue 作成時のフォーマット

```markdown
タイトル: refactor: [対象領域] の設計改善
本文:
## 現状の設計
[現在の実装の概要と問題点]
...
## 参考
session-retrospective で検出（YYYY-MM-DD）
```

ユーザーが承認した項目のみ `gh issue create` で作成する。
