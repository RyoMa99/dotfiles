...
調査が完了したら、結果を計画ファイルの `## 調査結果` セクションに書き出す。
...
調査結果を書き出したら、`AskUserQuestion` でユーザーにレビューを依頼する。
...
> `ExitPlanMode` する前に、計画ファイルが保存されていることを確認する（保存漏れがあると実装フェーズで参照できなくなるため）。

計画ファイルの保存は**タスク0**として計画に含める。実装フェーズで最初に実行されるタスクとして明示する。

#### 調査結果ファイルの並行保存

計画ファイルと並行して、Phase 2 の調査結果を `vX.Y.Z-research.md` として保存する。

- 計画ファイル: `docs/plan/vX.Y.Z.md`
- 調査結果ファイル: `docs/plan/vX.Y.Z-research.md`

計画ファイル内の `## 調査結果` セクションの内容を、そのまま research ファイルに書き出す。計画ファイルからは調査結果セクションを除外し、代わりに research ファイルへの参照を記載する。
