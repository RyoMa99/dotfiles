...
関連: `artifacts.md`（成果物の耐久性と管理方針）、`/rdra` スキル（上流の要求分析）
...
| 大きな差（10+ コミット、または変更ファイルが多い） | 計画に入らず、まず最新化を促す。古いベースで計画を立てると調査結果が実態と乖離し、手戻りが大きくなるため |
...
> Phase 1 を省略しない（ユーザーが明示的にスキップを指示した場合を除く）。調査対象が不明確なまま Phase 2 に入ると、無関係なコードを読む時間が無駄になる。

### 上流分析チェック

Phase 1 の意図明確化と並行して、上流の要求分析（RDRA）が必要かを判定する。

```
この機能のアクター・ゴール・業務フローは明確か？
    │
    ├─ 明確（既存 RDRA あり、または自明）
    │   → そのまま Phase 2 へ。RDRA 成果物を参照情報として活用
    │
    ├─ 部分的に不明確
    │   → 不足している箇所を特定し、AskUserQuestion で確認
    │   → RDRA の部分的な更新が必要な場合はユーザーに提案
    │
    └─ 不明確（新規システム、大規模新機能）
        → ExitPlanMode し、先に `/rdra` の実施を提案
        → RDRA 完了後に再度 EnterPlanMode
```

| 判定基準 | 対応 |
|---------|------|
| `docs/rdra/` に既存成果物があり、今回のスコープをカバーしている | RDRA 成果物を参照しつつ Phase 2 へ |
| 既存成果物はあるが今回のスコープが未カバー | `/rdra` で追記を提案（Step 4 の要件追加フローで対応可能） |
| 成果物がなく、かつ新規性が高い | `/rdra` を先に実施することを提案 |
| バグ修正・リファクタ・小規模改修 | RDRA 不要。Phase 2 へ |
...
> 性質は複合しうる（例: 探索的 + 高リスク = 新しい認証方式の導入）。複合する場合は最も厳格な性質を主として見積もりする（高リスク > 探索的 > 既知パターン）。

> 仮判定は Phase 2 の調査結果で見直す。ここでは初期仮説として設定する。

> **Gate**: ゴール・文脈・制約・スコープの4項目がユーザーと合意されていること（`AskUserQuestion` で明示的に確認）。上流分析の要否が判定されていること。
...
タスクが DB 変更を伴う可能性がある場合（マイグレーション、新テーブル、カラム追加等）、既存の DB 構造を把握する。

```bash
# ER 図で関連テーブルの全体像を把握
tbls out <DSN> -t mermaid

# .tbls.yml がある場合
tbls out -t mermaid
```

- 変更対象テーブルのリレーション・制約を確認してからタスク定義に入る
- `.tbls.yml` や `docs/schema/` が既にあればそちらを参照（`tbls out` は不要）
- DB 接続が不可能な場合はマイグレーションファイルやスキーマ定義ファイルから読み取る
...
- 既存の設計パターン・慣習
- 類似機能の既存実装（参考にできるコード）
- DB 変更がある場合: 既存テーブル構造、リレーション、制約、インデックス
- 以下の兆候がある場合は `/domain-modeling` スキルを参照: 同じ用語がアクターごとに異なる意味で使われている / 型が肥大化している / boolean フラグで状態管理している / 仕様表が複雑な組み合わせを持つ / 新しいドメイン概念が登場する
- 設計方針の選択肢（複数ある場合）
...
> **ADR 発火ポイント**: 複数の選択肢を比較して決定した場合、ADR として記録する。plan-mode 内ではファイル保存できないため、ADR の内容は会話内で確定し、ファイル保存は `/implementation` Step 0 で計画ファイルと一緒にコミットする。
...
変更対象領域について「この部分をゼロから設計するなら、どういう設計にしますか？」と問いかけ、現在の設計が新機能を綺麗に導入できる状態かを検証する。

| 乖離の程度 | 対応 |
|-----------|------|
| 小さい（現行設計で綺麗に入る） | そのまま Phase 3 に進む |
| 大きいが今回のスコープ外 | 設計負債として記録し、計画のリスク欄に明記 |
| 大きく新機能に直接影響 | 事前リファクタをスコープに含めることをユーザーに提案 |

> タスク性質が「既知パターン」で変更が小規模な場合はスキップしてよい。

### 調査結果の記録

調査が完了したら、結果をユーザーに提示する。
Claude がコードベースを誤解したまま計画に進むリスクを防ぐため、ユーザーレビューを挟む。

#### 記録すべき内容

- 変更対象ファイルと既存構造の概要
- 既存の設計パターン・慣習
- 類似実装の参照箇所（ファイルパス + 該当コードの要約）
- 設計選択肢がある場合はその比較
- RDRA 成果物がある場合、関連する REQ-xxx / UC-xxx / BUC-xxx の参照

#### レビュー依頼

調査結果を提示したら、`AskUserQuestion` でユーザーにレビューを依頼する。

- 「調査結果に誤解や不足はありませんか？」と確認
- ユーザーが誤解を指摘した場合は調査を修正し、再度レビューを依頼してから Phase 3 に進む
- ユーザーが OK した場合のみ Phase 3 に進む

> **Gate**: 変更対象ファイル・影響範囲が特定され、タスク性質が確定し、ユーザーが調査結果をレビュー済みであること
...
計画ファイルは**使い捨ての思考整理ツール**である（`artifacts.md` 参照）。

- 計画は会話内で作成する。ファイル保存・コミットは `/implementation` Step 0 で行う（plan-mode 内ではファイル操作不可）
- 実装完了後は PR description に要約を統合し、計画ファイルは削除する（`git log` で履歴は追える）
- 重要な設計判断は計画ファイルではなく `/adr` で記録する
...
タスク数ベースの粒度を、Phase 2 で確定したタスク性質で見積もりする。
...
**受入条件**（TDD のテストリストになる。RDRA の EARS 要件から GWT を導出する）:
- GIVEN [前提条件] WHEN [操作] THEN [期待動作]（正常系）
- GIVEN [前提条件] WHEN [境界条件] THEN [期待動作]（境界値）
- GIVEN [前提条件] WHEN [異常条件] THEN [エラー動作]（異常系）
※ GIVEN は前提が自明な場合は省略可
...
RDRA 成果物がある場合、計画のタスクから RDRA の要件への追跡可能性を維持する。

```
GOAL ← REQ ← UC ← タスク受入条件（GWT）
```

すべてのタスクが少なくとも1つの UC / REQ に紐づいていることを確認する（RDRA がない場合はスキップ）。

> **Gate**: 計画が作成済み、かつ全タスクに受入条件が定義されていること（自己チェック）
...
`ExitPlanMode` の前に実施する（レビューなしに計画を出すと、複雑性の見落としや一貫性の欠如が検出されないまま実装に入るリスクがある）。
...
計画の末尾に見積もりメモセクションを記録する。振り返り（session-retrospective）での検証材料になる。
