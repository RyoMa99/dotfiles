...
この機能のアクター・ゴール・業務フローは明確か？
    ...
    │
    └─ 不明確（新規システム、大規模新機能）
        → plan-mode を中断し、先に `/rdra` の実施を提案
        → RDRA 完了後に plan-mode を再開
```
...
- 設計選択肢がある場合はその比較
- RDRA 成果物がある場合、関連する REQ-xxx / BUC-xxx の参照
...
- `docs/plan/` に保存し、実装開始時にコミットする（git 履歴にスナップショットを残す）
...
```
GOAL ← REQ（EARS） ← タスク受入条件（GWT）
```

すべてのタスクが少なくとも1つの REQ に紐づいていることを確認する（RDRA がない場合はスキップ）。
...
計画の末尾に較正メモセクションを記録する。振り返り（session-retrospective）での検証材料になる。
