...
ユーザーが完了を確認した後、以下のスキルを実行する。

- `/record-business-knowledge` - 実装中に得た業務知識があれば記録を提案
- `/sync-knowledge` - 技術的な学びがあれば `~/.claude/rules/` に蓄積
- `/suggest-automation` - セッションのやりとりから自動化できるパターンを検出・提案

> いずれもユーザー承認なしに保存しない。提案のみ行い、不要なら即スキップ。
